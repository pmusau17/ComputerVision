...
trainPaths = sorted(list(paths.list_images(config.TRAIN_IMAGES)))

# the data is organized by synset id train/{synset}/images/synsetid_no.JPEG
...
# split the row in the annotation line with respect to tabs, so that each entry in the 
# file looks like the following [imagename.jpeg, label]

M = [r.split("\t")[:2] for r in M]

# create the full validation image file paths and get the labels too
