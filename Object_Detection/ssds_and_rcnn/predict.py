...
ap.add_argument("-m","--model",required=True,help="base path for frozen checkpoint graph")
...
ap.add_argument("-c","--min-confidence",type=float,default=0.5,help="minimum probability to filter weak detections")
...
# In order to predict bounding boxes for our input image we first need to create a TensorFlow session and grab references to each of the image
# bounding box, probability, and classes tensors inside the network
...
with model.as_default():
    with tf.Session(graph=model) as sess:
        ...
        # The feed dict instructs tensorflow to set the imageTensor to our image and run a forward pass, yielding our bounding boxes, scores (i.e probabilities)
        # and class labels
        ...
        # squeeze the list into a single dimension
